...
import { validateTranslatePayload } from './validate.js';
...
app.get('/health', (_req, res) => {
  res.json({
    ok: true,
    service: 'lingomate-backend',
    mode: process.env.LINGO_API_KEY ? 'configured' : 'demo'
  });
});

app.post('/translate', async (req, res) => {
  const body = req.body as Partial<TranslateRequest>;
  const validation = validateTranslatePayload(body);
  if (!validation.ok) {
